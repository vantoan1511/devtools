<template>
  <div class="flex h-[calc(100vh-60px)] flex-col bg-surface-50 dark:bg-surface-950 overflow-auto relative">
    <!-- Toolbar -->
    <DToolbar :pill="true">
      <template #start>
        <div
            class="flex items-center gap-2">
          <i class="pi pi-link text-primary animate-pulse"></i>
          <span class="font-black uppercase tracking-widest text-primary">Transformer Chain</span>
        </div>

        <Divider layout="vertical"/>

        <Select :options="transformerOptions" class="w-72 h-10 glass-select" optionGroupChildren="items"
                optionGroupLabel="label"
                optionLabel="label" optionValue="value" placeholder="Add Transformation Step..."
                @change="addStep">
          <template #optiongroup="slotProps">
            <div
                class="flex items-center gap-2 font-black text-[10px] uppercase tracking-[0.2em] text-surface-400 py-2 px-2">
          ...
          <template #option="slotProps">
            <div class="flex items-center gap-3 py-1">
              <div
                  :class="['w-8 h-8 rounded-lg flex items-center justify-center shrink-0', getCategoryColor(slotProps.option.category)]">
              ...
              <div class="flex flex-col gap-0.5">
                ...
                <div class="text-[10px] text-surface-500 leading-tight opacity-70">{{
      ...
      <template #end>
        <div class="flex items-center gap-2">
          <Button v-tooltip.bottom="'Clear Everything'" icon="pi pi-trash" rounded severity="secondary" text
                  @click="clearAll"/>
        </div>
      </template>
    </DToolbar>
    ...
    <div class="flex-1 flex flex-col lg:flex-row">
      <div
          class="w-full lg:w-[380px] p-6 border-r border-surface-200 dark:border-white/10 flex flex-col gap-6 overflow-y-auto bg-white/20 dark:bg-surface-900/10 backdrop-blur-sm">
        <!-- Initial Input -->
        ...
        <div class="flex flex-col gap-4 relative">
          <div class="flex items-center justify-between px-1">
            ...
            <Tag :value="selectedSteps.length + ' steps'" severity="primary"
                 class="text-[9px] font-black px-2 rounded-full"/>
          </div>
          ...
          <div class="relative flex flex-col gap-3">
            ...
            <TransitionGroup name="pipeline" tag="div" class="flex flex-col gap-3 relative z-10">
              <div v-for="(stepId, index) in selectedSteps" :key="index + stepId"
                ...
                <div class="flex flex-col items-center gap-1 shrink-0">
                  ...
                  <div
                      class="w-6 h-6 rounded-full bg-primary/10 text-primary text-[10px] font-black flex items-center justify-center border border-primary/20 shadow-inner">
                    {{ index + 1 }}
                  </div>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-0.5">
                    <div
                        :class="['w-5 h-5 rounded flex items-center justify-center shrink-0', getCategoryColor(getStepTransformer(stepId)?.category || '')]">
            ...
            <div v-if="selectedSteps.length === 0"
              ...
              <div
                  class="w-16 h-16 rounded-full bg-surface-100 dark:bg-surface-800/50 flex items-center justify-center group-hover:scale-110 group-hover:rotate-12 transition-transform duration-500">
      ...
      <div class="flex-1 p-6 md:p-8 overflow-y-auto bg-surface-100/20 dark:bg-surface-950/20 relative">
        <div class="max-w-4xl mx-auto flex flex-col gap-10">
          <!-- Final Result Card -->
          <Card
              class="border-none shadow-xl shadow-black/5 dark:shadow-black/20 bg-white dark:bg-surface-900 rounded-[2rem] overflow-hidden group/result">
            <template #title>
              <div class="flex items-center justify-between px-4 pt-4">
                <div class="flex items-center gap-3">
                  <div
                      class="w-10 h-10 rounded-2xl bg-green-500/10 flex items-center justify-center ring-1 ring-green-500/20 transition-transform group-hover/result:rotate-6">
          ...
          <div v-if="results.steps.length > 0" class="flex flex-col gap-6 animate-in">
            <div class="flex items-center gap-3 px-2">
              ...
              <span
                  class="text-[10px] font-black uppercase tracking-[0.3em] text-surface-400">Step-by-Step Breakdown</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="(res, idx) in results.steps" :key="idx"
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <span
                        class="w-6 h-6 rounded-xl bg-primary/10 text-primary text-[10px] font-black flex items-center justify-center ring-1 ring-primary/20">
                      {{ idx + 1 }}
                    </span>
                    <div class="flex flex-col">
                ...
                <div v-if="res.error"
                     class="p-2.5 rounded-xl bg-red-500/5 border border-red-500/20 flex items-center gap-2">
                ...
                <div
                    class="font-mono text-[11px] p-4 rounded-2xl bg-surface-50/50 dark:bg-surface-950/50 text-surface-600 dark:text-surface-400 break-all max-h-40 overflow-y-auto border border-surface-100 dark:border-surface-800/50 leading-relaxed">
